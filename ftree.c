...
// Use linked list to store multiple clients.
...
// Client implement function, deal client's file/directory infomations and
...
int rcopy_client_impl (char *source, char *host, unsigned short port,
	...
	if (S_ISREG(src_info.st_mode)) {
		...
		for (dp = readdir(dirp); dp != NULL; dp = readdir(dirp)) {
			...
			// Build a correct path as recursion input, like A3.
			char* path = build_path(source, dp->d_name);
			rcopy_client_impl(path, host, port, sock_fd);

			// Because we use malloc in build_path, so we should free it.
			free(path);
		}
	}
...
void handle_sendfile (struct request *info, char *source, char *host,
	...
	if (result < 0) {
		...
		// Connect to the server.
		if (connect(sock_fd_child, (struct sockaddr *) &server, sizeof(server))
				== -1) {
			perror("client: connect");
			close(sock_fd_child);
			exit(1);
...
void handleclient_dir(struct client* p) {
	...
	// Get info of dir.
	struct stat info;
	int existence = lstat(dest_dir, &info);
	DIR * dirp;
